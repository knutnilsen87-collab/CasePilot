...
assert.match(appSource, /suppressProgressActions=\{false\}/, "CaseRoom progress actions prop is wired to App.tsx");
...
assert.match(appSource, /onSetVisualMode/, "merged CaseHeader receives visual mode control from App");
assert.match(appSource, /PrivacyPreAlphaNotice/, "pre-alpha notice component is mounted for first-time users");
assert.match(appSource, /Gå til Dokumentkontroll/, "needs-review import state has one clear primary next action");
...
assert.match(appSource, /!isCaseRoomView/, "CaseHeader is hidden on caseRoom screen to keep chat surface clean");
